...
export class ContactsClient extends GatewayClient {
    ...
    async listContacts(
        ...
        return this.request<ContactListResponse>(`/api/v1/contacts?${params}`);
    }
    ...
    async getContact(contactId: string, userId: string): Promise<ContactResponse> {
        ...
        return this.request<ContactResponse>(`/api/v1/contacts/${contactId}?${params}`);
    }
    ...
    async deleteContact(contactId: string, userId: string): Promise<{ success: boolean; message: string }> {
        const params = new URLSearchParams({ user_id: userId });
        return this.request<{ success: boolean; message: string }>(`/api/v1/contacts/${contactId}?${params}`, {
            method: 'DELETE',
        });
    }
    ...
    async searchContacts(
        ...
        return this.request<Contact[]>(`/api/v1/contacts/search?${params}`);
    }
    ...
    async getContactStats(userId: string): Promise<ContactStatsResponse> {
        const params = new URLSearchParams({ user_id: userId });
        return this.request<ContactStatsResponse>(`/api/v1/contacts/stats?${params}`);
    }
