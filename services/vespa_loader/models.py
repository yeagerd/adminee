...
class OfficeRouterDocument(BaseModel):
    ...
    document_type: str = Field(..., description="Document type (e.g., 'briefly_document')")
...
class VespaDocumentFields(BaseModel):
    ...
    source_type: str = Field(..., description="Type of document (e.g., 'email', 'calendar', 'contact')")
    ...
    updated_at: Optional[Union[str, datetime]] = Field(None, description="Last update timestamp")
...
class VespaLoaderExpectedDocument(BaseModel):
    ...
    user_id: str = Field(..., description="User ID for the document")
    id: str = Field(..., description="Document ID (note: different from doc_id in router)")
    provider: str = Field(..., description="Data provider")
    type: str = Field(..., description="Document type (note: different from source_type in router)")
    subject: Optional[str] = Field(None, description="Document subject (note: different from title in router)")
    body: Optional[str] = Field(None, description="Document body (note: different from content in router)")
    sender: Optional[str] = Field(None, description="Document sender (note: different from sender in router)")
    to: Optional[List[str]] = Field(None, description="Document recipients (note: different from recipients in router)")
...
class VespaIndexedDocument(BaseModel):
    ...
    recipients: List[str] = Field(default_factory=list, description="Document recipients")
