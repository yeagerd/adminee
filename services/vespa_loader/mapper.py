...
class DocumentMapper:
    ...
    def _determine_document_type(self, document_data: Dict[str, Any]) -> str:
        ...
        if "display_name" in document_data and (
            "email_addresses" in document_data or "phone_numbers" in document_data
        ):
            return "contact"
    ...
    def _map_fields(
        self, source: Dict[str, Any], target: Dict[str, Any], mappings: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        if doc_type == "email":
            ...
            if vespa_doc.get("attendees"):
                if isinstance(vespa_doc["attendees"], list):
                    search_parts.append(
                        f"Attendees: {', '.join(vespa_doc['attendees'])}"
                    )
                else:
    ...
    def _extract_metadata(
        self, document_data: Dict[str, Any], doc_type: str
    ) -> Dict[str, Any]:
    ...
    def _validate_vespa_document(self, vespa_doc: Dict[str, Any]) -> None:
        ...
        missing_fields = [
            field for field in required_fields if not vespa_doc.get(field)
        ]

        if missing_fields:
            raise ValueError(
                f"Missing required fields for Vespa document: {missing_fields}"
            )
        ...
        for field in timestamp_fields:
            if vespa_doc.get(field) and isinstance(vespa_doc[field], str):
                try:
                    ...
                    logger.warning(
                        f"Invalid timestamp format for {field}: {vespa_doc[field]}"
                    )
        ...
        logger.debug(
            f"Vespa document validation passed for {vespa_doc.get('source_type')} document"
        )
    ...
    def map_batch(self, documents: list[Dict[str, Any]]) -> list[Dict[str, Any]]:
        ...
        logger.info(
            f"Successfully mapped {len(mapped_documents)} out of {len(documents)} documents"
        )
