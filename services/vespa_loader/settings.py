...
class Settings(BaseSettings):
    ...
    api_frontend_vespa_loader_key: str = Field(..., validation_alias="API_FRONTEND_VESPA_LOADER_KEY")
    api_vespa_loader_user_key: str = Field(..., validation_alias="API_VESPA_LOADER_USER_KEY")
    api_vespa_loader_office_key: str = Field(..., validation_alias="API_VESPA_LOADER_OFFICE_KEY")
    ...
    # Content processing
    max_content_length: int = Field(default=10000, validation_alias="MAX_CONTENT_LENGTH")
    enable_html_stripping: bool = Field(default=True, validation_alias="ENABLE_HTML_STRIPPING")
    enable_email_header_cleaning: bool = Field(
        default=True, validation_alias="ENABLE_EMAIL_HEADER_CLEANING"
    )

    # Batch processing
    max_concurrent_batches: int = Field(default=5, validation_alias="MAX_CONCURRENT_BATCHES")
    ...
    retry_delay_seconds: float = Field(default=1.0, validation_alias="RETRY_DELAY_SECONDS")

    # Rate limiting
    max_documents_per_second: int = Field(default=100, validation_alias="MAX_DOCUMENTS_PER_SECOND")
    ...
    pubsub_project_id: str = Field(default="briefly-dev", validation_alias="PUBSUB_PROJECT_ID")
    pubsub_emulator_host: str = Field(
        default="localhost:8085", validation_alias="PUBSUB_EMULATOR_HOST"
    )
    enable_pubsub_consumer: bool = Field(default=True, validation_alias="ENABLE_PUBSUB_CONSUMER")
