...
class TestIDCorruptionDetection(BaseSelectiveHTTPIntegrationTest):
            ...
            def post(self, url, json=None):
                ...
                if "/document/v1/" in url:
                    ...
                    if self.corruption_mode:
                        ...
                        corrupted_id = f"id:briefly:briefly_document:g={user_id}:{doc_id}"
    ...
    def test_detect_id_corruption_pattern(self):
        ...
        for corrupted_id in corrupted_ids:
            ...
            assert re.search(corruption_pattern, corrupted_id), f"Failed to detect corruption in: {corrupted_id}"
            
            # Should have the corruption pattern (note: it's :: not ::)
            assert corrupted_id.count("id:briefly:briefly_document::") == 1, f"Expected corruption pattern in: {corrupted_id}"
            assert "id:briefly:briefly_document::id:briefly:briefly_document:g=" in corrupted_id, f"Expected corruption pattern in: {corrupted_id}"
    ...
    def test_correct_id_format_validation(self):
        ...
        for correct_id in correct_ids:
            ...
            assert not re.search(corruption_pattern, correct_id), f"False positive corruption detection in: {correct_id}"
            
            # Should have single pattern
            assert correct_id.count("id:briefly:briefly_document:g=") == 1, f"Expected single pattern in: {correct_id}"
                ...
                if "id:briefly:briefly_document:g=" in corrupted_id:
                    ...
                    if len(parts) > 1:
                        ...
                        if ":" in user_doc_part:
                            ...
                            pytest.fail(f"Could not parse user:doc from corrupted ID: {corrupted_id}")
                    else:
                        pytest.fail(f"Could not find g= part in corrupted ID: {corrupted_id}")
                else:
                    pytest.fail(f"Could not find streaming mode pattern in corrupted ID: {corrupted_id}")
                    
            except Exception as e:
                pytest.fail(f"Clear script parsing failed for corrupted ID {corrupted_id}: {e}")
    ...
    def test_corruption_prevention_validation(self):
        ...
        for corrupted_id in corrupted_ids:
            assert not validate_vespa_id(corrupted_id), f"Corrupted ID should fail validation: {corrupted_id}"
