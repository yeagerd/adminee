...
class TestUserIsolation(BaseSelectiveHTTPIntegrationTest):
    ...
    def test_user_id_field_presence_critical(self):
        ...
        assert (
            "user_id" in fields
        ), "user_id field is missing - this breaks user isolation!"
        ...
        assert re.match(
            email_pattern, fields["user_id"]
        ), f"user_id '{fields['user_id']}' is not a valid email format"
    ...
    def test_user_isolation_through_user_id_field(self):
        ...
        for result in user1_results:
            assert (
                result["fields"]["user_id"] == "user1@example.com"
            ), "User 1 result contains wrong user_id"

        for result in user2_results:
            assert (
                result["fields"]["user_id"] == "user2@example.com"
            ), "User 2 result contains wrong user_id"
        ...
        assert (
            "user2@example.com" not in user1_ids
        ), "User 1 results contaminated with User 2 data"
        assert (
            "user1@example.com" not in user2_ids
        ), "User 2 results contaminated with User 1 data"
    ...
    def test_user_id_field_immutability(self):
        ...
        assert (
            original_doc["user_id"] != modified_doc["user_id"]
        ), "user_id was modified - security breach!"

        # Original user_id should remain unchanged
        assert (
            original_doc["user_id"] == "original@example.com"
        ), "Original user_id was corrupted"
    ...
    def test_user_id_field_format_validation(self):
        ...
        for user_id in valid_user_ids:
            assert self._is_valid_email(
                user_id
            ), f"Valid email '{user_id}' was rejected"
        ...
        for user_id in invalid_user_ids:
            if user_id is not None:  # Skip None for assertion
                assert not self._is_valid_email(
                    user_id
                ), f"Invalid email '{user_id}' was accepted"
    ...
    async def test_indexing_preserves_user_id(self, vespa_client):
        ...
        assert (
            test_doc["user_id"] == "test@example.com"
        ), "user_id was corrupted during test"
    ...
    def test_search_query_user_isolation(self):
        ...
        for query in test_queries:
            ...
            assert (
                "select * from briefly_document where" in query
            ), f"Invalid query structure: {query}"
    ...
    def test_document_retrieval_user_isolation(self):
        ...
        user2_docs = [{"fields": {"user_id": "user2@example.com", "doc_id": "doc_3"}}]

        # Test that user1 can only see their own documents
        user1_visible_docs = [
            doc
            for doc in user1_docs + user2_docs
            if doc["fields"]["user_id"] == "user1@example.com"
        ]
        ...
        user2_visible_docs = [
            doc
            for doc in user1_docs + user2_docs
            if doc["fields"]["user_id"] == "user2@example.com"
        ]
        assert len(user2_visible_docs) == 1, "User 2 should only see 1 document"

        # Verify no cross-access
        user1_cross_access = [
            doc for doc in user2_docs if doc["fields"]["user_id"] == "user1@example.com"
        ]
        user2_cross_access = [
            doc for doc in user1_docs if doc["fields"]["user_id"] == "user2@example.com"
        ]
    ...
    def test_missing_user_id_security_implications(self):
        ...
        assert (
            fields["user_id"].strip() != ""
        ), "Security fix: user_id field is not empty"

        # Verify the security fix is working
        assert (
            fields["user_id"] == "test@example.com"
        ), "Security fix: user_id field contains correct value"
