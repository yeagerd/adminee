...
class TestDataStructureMismatch:
    ...
    def test_office_router_data_conversion(self):
        ...
        from services.vespa_loader.models import validate_office_router_document, convert_office_router_to_vespa_loader
    ...
    async def test_process_document_handles_nested_data(self):
        ...
        with patch('services.vespa_loader.main.content_normalizer', mock_content_normalizer), \
            ...
            assert call_args["sender"] == "sender@example.com"  # sender mapped to sender
            assert call_args["to"] == ["recipient@example.com"]  # recipients mapped to to
    ...
    def test_office_router_vs_vespa_loader_data_format_mismatch(self):
        ...
        assert office_router_format["fields"]["doc_id"] == vespa_loader_expected["id"]  # Same value, different field names
        assert office_router_format["fields"]["source_type"] == vespa_loader_expected["type"]  # Same value, different field names
        assert office_router_format["fields"]["title"] == vespa_loader_expected["subject"]  # Same value, different field names
        assert office_router_format["fields"]["content"] == vespa_loader_expected["body"]  # Same value, different field names
        assert office_router_format["fields"]["sender"] == vespa_loader_expected["sender"]  # Same value, different field names
        assert office_router_format["fields"]["recipients"] == vespa_loader_expected["to"]  # Same value, different field names
