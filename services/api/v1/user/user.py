...
class UserBase(BaseModel):
    ...
    def validate_names(cls: type["UserBase"], v: Any) -> Any:
        ...
        # Basic validation - check for empty strings and length
        if isinstance(v, str):
            v = v.strip()
            if len(v) > 100:
    ...
    def validate_profile_image_url(cls: type["UserBase"], v: Any) -> Any:
        ...
        if isinstance(v, str) and (v.startswith("http://") or v.startswith("https://")):
            return v
        raise ValueError("Profile image URL must be a valid HTTP/HTTPS URL")
    ...
    def validate_email_enhanced(cls: type["UserBase"], v: Any) -> Any:
        ...
        # Pydantic EmailStr already handles basic validation
        return v
...
class UserCreate(UserBase):
    ...
    def validate_external_auth_id(cls: type["UserCreate"], v: Any) -> Any:
        ...
        v = v.strip()
        if len(v) > 255:
            raise ValueError("External auth ID must be 255 characters or less")
        return v
...
class UserOnboardingUpdate(BaseModel):
    ...
    def validate_onboarding_step(cls: type["UserOnboardingUpdate"], v: Any) -> Any:
        ...
        if isinstance(v, str):
            v = v.strip()
            if len(v) > 50:
                raise ValueError("Onboarding step must be 50 characters or less")
