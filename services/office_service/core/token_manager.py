...
class CachedToken:
    ...
    def __init__(self, token_data: TokenData, ttl_seconds: int = 900):  # 15 minutes default
...
class TokenManager:
    ...
    def _generate_cache_key(self, user_id: str, provider: str, scopes: List[str]) -> str:
    ...
    async def _set_cache(self, cache_key: str, token_data: TokenData, ttl_seconds: int = 900):
    ...
    async def _cleanup_expired_tokens(self):
        ...
        async with self._cache_lock:
            expired_keys = [
                key for key, cached_token in self._token_cache.items()
    ...
    async def get_user_token(self, user_id: str, provider: str, scopes: List[str]) -> Optional[TokenData]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Successfully retrieved token for user {user_id}, provider {provider}")
                return token_data
            
            elif response.status_code == 404:
                logger.warning(f"No token found for user {user_id}, provider {provider}")
                return None
            
            elif response.status_code == 403:
                logger.warning(f"Insufficient permissions for user {user_id}, provider {provider}")
                return None
            
            else:
                logger.error(f"Token retrieval failed with status {response.status_code}: {response.text}")
                return None
                
        except httpx.TimeoutException:
            logger.error(f"Token retrieval timed out for user {user_id}, provider {provider}")
            return None
        
        except httpx.NetworkError as e:
            logger.error(f"Network error during token retrieval for user {user_id}, provider {provider}: {e}")
            return None
        
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error during token retrieval for user {user_id}, provider {provider}: {e}")
            return None
        
        except Exception as e:
            logger.error(f"Unexpected error during token retrieval for user {user_id}, provider {provider}: {e}")
            return None
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        expired_tokens = sum(1 for cached_token in self._token_cache.values() if cached_token.is_expired())
