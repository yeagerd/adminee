...
from services.office_service.schemas import CalendarEvent, ApiResponse, PaginatedResponse
...
async def get_calendar_events(
    ...
    limit: int = Query(50, ge=1, le=200, description="Maximum number of events to return per provider"),
    start_date: Optional[str] = Query(None, description="Start date for event range (ISO format: YYYY-MM-DD)"),
    end_date: Optional[str] = Query(None, description="End date for event range (ISO format: YYYY-MM-DD)"),
    calendar_ids: Optional[List[str]] = Query(None, description="Specific calendar IDs to fetch from"),
    ...
    logger.info(f"[{request_id}] Calendar events request: user_id={user_id}, providers={providers}, limit={limit}")
    ...
    try:
        ...
        for provider in valid_providers:
            task = fetch_provider_events(
                request_id, user_id, provider, limit, start_dt, end_dt, calendar_ids, q, time_zone
            )
            tasks.append(task)
        ...
        for i, result in enumerate(provider_results):
            ...
            if isinstance(result, Exception):
                ...
                logger.info(f"[{request_id}] Provider {provider} returned {len(events)} events")
        ...
        logger.info(f"[{request_id}] Calendar events request completed in {response_time_ms}ms")
...
async def get_calendar_event(
    ...
    logger.info(f"[{request_id}] Calendar event detail request: event_id={event_id}, user_id={user_id}")
    ...
    try:
        ...
        event = await fetch_single_event(request_id, user_id, provider, original_event_id)
        ...
        logger.info(f"[{request_id}] Event detail request completed in {response_time_ms}ms")
...
async def fetch_provider_events(
    ...
    try:
        ...
        if provider == "google":
            ...
            for calendar_id in calendars_to_query:
                try:
                    ...
                    events_response = await client.get_calendar_events(calendar_id, **params)
                    ...
                    logger.warning(f"[{request_id}] Failed to fetch from Google calendar {calendar_id}: {calendar_error}")
                    continue
            ...
            for event_data in events:
                ...
                google_format_event = convert_microsoft_event_to_google_format(event_data)
        ...
        logger.info(f"[{request_id}] Successfully fetched {len(normalized_events)} events from {provider}")
...
async def fetch_single_event(
    ...
    try:
        ...
        if provider == "google":
            ...
            return normalize_google_calendar_event(event, account_email, account_name, calendar_name)
        ...
        logger.error(f"[{request_id}] Failed to fetch event {original_event_id} from {provider}: {e}")
        return None


def convert_microsoft_event_to_google_format(ms_event: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    google_event = {
        ...
        "visibility": "private" if ms_event.get("sensitivity") == "private" else "default",
    ...
    for attendee in ms_event.get("attendees", []):
        ...
        attendees.append({
            ...
            "responseStatus": attendee.get("status", {}).get("response", "needsAction")
        })
