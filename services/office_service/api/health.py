...
async def integration_health_check(
    ...
    try:
        ...
        integration_checks["microsoft"] = await check_user_integration(
            user_id, "microsoft"
        )
        ...
        response_data = {
            ...
            "status": (
                "all_healthy"
                if all_healthy
                else ("partial" if any_healthy else "unhealthy")
            ),
            "timestamp": start_time.isoformat(),
            "response_time_ms": response_time_ms,
        ...
        logger.info(
            f"Integration health check for user {user_id}: {response_data['status']}"
        )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if is_healthy:
                ...
                logger.warning(
                    f"Service health check failed for {service_url}: {response.status_code}"
                )
...
async def check_user_integration(user_id: str, provider: str) -> Dict[str, Any]:
    ...
    try:
        ...
        scopes = (
            ...
            if provider == "google"
            else ["https://graph.microsoft.com/user.read"]
        )
        ...
        if token_data and token_data.access_token:
            ...
            integration_info["token_expires_at"] = (
                token_data.expires_at.isoformat() if token_data.expires_at else None
            )
            logger.debug(
                f"Integration check passed for user {user_id}, provider {provider}"
            )
        else:
            integration_info["error"] = "No valid token available"
            logger.warning(
                f"Integration check failed for user {user_id}, provider {provider}: No valid token"
            )

    except Exception as e:
        integration_info["error"] = str(e)
        logger.error(
            f"Integration check failed for user {user_id}, provider {provider}: {e}"
        )
