...
def test_process_gmail_notification_valid(monkeypatch):
    ...
    with patch("gmail_sync_service.GmailAPIClient") as MockClient:
        mock_client = MockClient.return_value
        mock_client.fetch_emails_since_history_id.return_value = [{"id": "email1"}, {"id": "email2"}]
        with patch("gmail_sync_service.publish_message") as mock_publish:
...
def test_process_gmail_notification_pubsub_failure(monkeypatch):
    ...
    with patch("gmail_sync_service.GmailAPIClient") as MockClient:
        ...
        with patch("gmail_sync_service.publish_message", side_effect=Exception("pubsub error")) as mock_publish:
            process_gmail_notification(msg)
            assert msg.acked
            assert not msg.nacked
