...
def upgrade():
    op.add_column(
        'user_preferences',
        sa.Column('timezone_mode', sa.String(length=10), nullable=False, server_default='auto')
    )
    op.add_column(
        'user_preferences',
        sa.Column('manual_timezone', sa.String(length=50), nullable=False, server_default='')
    )
