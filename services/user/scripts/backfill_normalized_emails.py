    ...
    async with async_session() as session:
        ...
        while offset < total_users:
            ...
            result = await session.execute(
                select(User)
                .where(User.normalized_email.is_(None))  # Only process users without normalized_email
                .limit(batch_size)
                .offset(offset)
            )
            ...
            for user in users:
                try:
                    ...
                    logger.error(f"Failed to normalize email for user {user.id} ({user.email}): {e}")
                    errors += 1
...
async def verify_backfill() -> None:
    ...
    async with async_session() as session:
        ...
        logger.info(f"Verification: {total_users - users_without_normalized}/{total_users} users have normalized_email")
        
        if users_without_normalized > 0:
            logger.warning(f"{users_without_normalized} users still missing normalized_email")
        else:
