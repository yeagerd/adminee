...
class ContactDiscoveryConsumer:
    ...
    def _consume_topic(
        ...
        def callback(message: ReceivedMessage) -> None:
            try:
                logger.debug(f"Processing message from {topic_name}: {message.message_id}")
                ...
                message.ack()
                logger.debug(f"Successfully processed message {message.message_id} from {topic_name}")
                
            except Exception as e:
                logger.error(f"Error processing message {message.message_id} from {topic_name}: {e}")
    ...
    def _process_email_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed email event for contact discovery: {email_event.email.message_id}")
    ...
    def _process_calendar_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed calendar event for contact discovery: {calendar_event.event.id}")
    ...
    def _process_contact_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed contact event for contact discovery: {contact_event.contact.id}")
    ...
    def _process_document_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed document event for contact discovery: {document_event.document.id}")
    ...
    def _process_todo_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(f"Processed todo event for contact discovery: {todo_event.todo.id}")
