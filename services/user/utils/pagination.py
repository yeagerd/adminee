...
This module provides user-specific cursor-based pagination functionality
...
class UserCursorPagination(BaseCursorPagination):
    """
    User-specific cursor-based pagination implementation.
    
    This class extends the base cursor pagination with user-specific
    functionality for user management and search.
    """
    ...
    def build_query_filters(self, cursor_info: CursorInfo) -> Dict[str, Any]:
        ...
        if cursor_info.direction == "next":
            ...
            filters["cursor_params"] = {
                "last_created_at": cursor_info.last_timestamp,
        ...
        if "onboarding_completed" in user_filters and user_filters["onboarding_completed"] is not None:
    ...
    def validate_user_filters(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "onboarding_completed" in filters and filters["onboarding_completed"] is not None:
            # Convert to boolean if it's a string
            onboarding_completed = filters["onboarding_completed"]
            if isinstance(onboarding_completed, str):
                onboarding_completed = onboarding_completed.lower() in ("true", "1", "yes")
            validated_filters["onboarding_completed"] = bool(onboarding_completed)
