...
class ParsedTrackingInfo(BaseModel):
    ...
    source: str = Field(..., description="Where this information was found: 'subject', 'body', or 'both'")


class EmailParseResponse(BaseModel):
    """Response schema for email parsing"""
    is_shipment_email: bool = Field(..., description="Whether this appears to be a shipment email")
    detected_carrier: Optional[str] = Field(None, description="Primary detected carrier")
    tracking_numbers: List[ParsedTrackingInfo] = Field(default_factory=list, description="List of tracking numbers found")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Overall confidence score")
    detected_from: str = Field(..., description="Where detection was based: 'sender', 'subject', 'body', or 'multiple'")
    suggested_package_data: Optional[dict] = Field(None, description="Suggested package data for creating tracking entry")
