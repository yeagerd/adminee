...
class TestVespaDataFlow:
    ...
    async def setup(self):
        ...
        self.pubsub_publisher = PubSubPublisher(
            self.config["pubsub_project_id"],
            self.config["pubsub_emulator_host"]
        )
    ...
    async def test_complete_data_flow(self):
        ...
        logger.info(f"Search test successful: {search_results['total_found']} results found")
        
        # Phase 5: Test user isolation
        isolation_test = await self._test_user_isolation()
        
        assert isolation_test["passed"], f"User isolation test failed: {isolation_test['error']}"
        logger.info("User isolation test passed")
        
        # Phase 6: Test data consistency
        consistency_test = await self._test_data_consistency(test_data, indexed_documents)
        
        assert consistency_test["passed"], f"Data consistency test failed: {consistency_test['error']}"
        ...
        for data_item in test_data:
            try:
                if data_item["type"] == "email":
                    ...
                    message_id = await self.pubsub_publisher.publish_calendar_event(data_item)
                ...
                logger.error(f"Failed to publish {data_item['type']} {data_item['id']}: {e}")
        
        return published_messages
    
    async def _wait_for_vespa_indexing(self, expected_count: int, timeout_seconds: int = 60):
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                indexed_count = results.get("root", {}).get("fields", {}).get("totalCount", 0)
                
                if indexed_count >= expected_count:
                    logger.info(f"Indexing completed: {indexed_count} documents found")
                    return
                
                logger.info(f"Waiting for indexing... {indexed_count}/{expected_count} documents found")
        ...
        raise TimeoutError(f"Timeout waiting for Vespa indexing. Expected {expected_count} documents.")
    
    async def _verify_vespa_indexing(self, test_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _test_search_functionality(self, test_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _test_user_isolation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_user_count = test_user_results.get("root", {}).get("fields", {}).get("totalCount", 0)
            ...
            other_user_count = other_user_results.get("root", {}).get("fields", {}).get("totalCount", 0)
    ...
    async def _test_data_consistency(self, original_data: List[Dict[str, Any]], 
                                   indexed_documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for original_item in original_data:
                ...
                indexed_item = next(
                    (item for item in indexed_documents if item["id"] == original_item["id"]), 
                    None
                )
            ...
            consistent_checks = len([c for c in consistency_checks if c["status"] == "consistent"])
            consistency_score = consistent_checks / total_checks if total_checks > 0 else 0
    ...
    async def test_performance_and_scalability(self):
        ...
        logger.info(f"Batch search time: {batch_time:.3f}s for {len(batch_queries)} queries")
        
        # Test 3: Result quality
        assert len(batch_results) == len(batch_queries), "Batch search should return results for all queries"
        
        successful_queries = sum(1 for r in batch_results if "error" not in r)
        success_rate = successful_queries / len(batch_queries)
        
        assert success_rate >= 0.8, f"Batch search success rate too low: {success_rate:.1%}"
