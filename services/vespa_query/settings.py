...
class Settings(BaseSettings):
    ...
    # Vespa configuration
    vespa_endpoint: str = Field(default="http://localhost:8080", validation_alias="VESPA_ENDPOINT")
    vespa_timeout: int = Field(default=30, validation_alias="VESPA_TIMEOUT")
    vespa_application: str = Field(default="briefly", validation_alias="VESPA_APPLICATION")
    ...
    max_concurrent_queries: int = Field(default=20, validation_alias="MAX_CONCURRENT_QUERIES")
    enable_query_caching: bool = Field(default=True, validation_alias="ENABLE_QUERY_CACHING")

    # Result processing
    enable_facets: bool = Field(default=True, validation_alias="ENABLE_FACETS")
    enable_highlighting: bool = Field(default=True, validation_alias="ENABLE_HIGHLIGHTING")
    max_facet_values: int = Field(default=50, validation_alias="MAX_FACET_VALUES")

    # Performance tuning
    connection_pool_size: int = Field(default=10, validation_alias="CONNECTION_POOL_SIZE")
    ...
    health_check_interval_seconds: int = Field(default=30, validation_alias="HEALTH_CHECK_INTERVAL")
    health_check_timeout_seconds: int = Field(default=5, validation_alias="HEALTH_CHECK_TIMEOUT")

    # Security
    enable_user_isolation: bool = Field(default=True, validation_alias="ENABLE_USER_ISOLATION")
    ...
    api_frontend_vespa_query_key: str = Field(..., validation_alias="API_FRONTEND_VESPA_QUERY_KEY")
    api_vespa_query_user_key: str = Field(..., validation_alias="API_VESPA_QUERY_USER_KEY")
    api_vespa_query_office_key: str = Field(..., validation_alias="API_VESPA_QUERY_OFFICE_KEY")
    ...
    office_service_url: str = Field(
        default="http://localhost:8003", validation_alias="OFFICE_SERVICE_URL"
    )
    user_service_url: str = Field(
        default="http://localhost:8001", validation_alias="USER_SERVICE_URL"
    )
