...
class MicrosoftAPIClient(BaseAPIClient):
    ...
    async def create_draft_message(self, message_data: Dict[str, Any]) -> Dict[str, Any]:
        """Create a new draft message in Drafts folder."""
        response = await self.post("/me/messages", json_data=message_data)
        return response.json()

    async def create_reply_draft(self, message_id: str, reply_all: bool = False) -> Dict[str, Any]:
        """Create a draft reply or reply-all for a message."""
        endpoint = f"/me/messages/{message_id}/createReplyAll" if reply_all else f"/me/messages/{message_id}/createReply"
        response = await self.post(endpoint, json_data={})
        return response.json()

    async def create_forward_draft(self, message_id: str) -> Dict[str, Any]:
        response = await self.post(f"/me/messages/{message_id}/createForward", json_data={})
        return response.json()

    async def get_message_draft(self, draft_id: str) -> Dict[str, Any]:
        return await self.get_message(draft_id)

    async def update_draft_message(self, draft_id: str, patch_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def list_drafts_by_conversation(self, conversation_id: str, top: int = 50, skip: int = 0) -> Dict[str, Any]:
        params: Dict[str, Any] = {
            "$top": top,
            "$skip": skip,
