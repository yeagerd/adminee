...
async def test_realistic_concurrent_access():
    ...
    with (
        patch("services.office.core.api_client_factory.get_settings") as mock_settings,
        ...
        with patch(
            "services.office.core.api_client_factory.TokenManager", MockTokenManager
        ):

            # Simulate realistic concurrent scenarios
            async def realistic_client_task(
                task_id: int, provider: str, delay: float = 0
            ):
                ...
                try:
                    ...
                    print(
                        f"Task {task_id}: {provider} client creation completed successfully"
                    )
                    return True
            ...
            for i in range(3, 5):
                task = asyncio.create_task(
                    realistic_client_task(i, "google", delay=0.1)
                )
                tasks.append(task)

            # Mixed provider tasks with different delays
            task = asyncio.create_task(
                realistic_client_task(5, "microsoft", delay=0.05)
            )
            tasks.append(task)
            ...
            assert (
                failed_tasks == 0
            ), f"Expected all tasks to succeed, but {failed_tasks} failed"
...
async def test_ref_count_validation():
    ...
    with (
        ...
        with patch(
            ...
            tasks = [asyncio.create_task(staggered_task(i, i * 0.1)) for i in range(3)]
            await asyncio.gather(*tasks)
...
async def test_error_handling():
    ...
    with (
        ...
        with patch(
            ...
            print(
                f"Mixed success/failure test: {success_count} succeeded, {failure_count} failed"
            )
