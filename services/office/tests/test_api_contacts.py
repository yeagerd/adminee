...
class TestContactsApi:
    ...
    async def test_list_contacts_success(self, client, auth_headers):
        with patch("services.office.api.contacts.cache_manager") as mock_cache, \
             patch("services.office.api.contacts.get_api_client_factory") as mock_factory:
            ...
            class FakeGoogle:
                async def get_contacts(self, page_size=200):
                    return {"connections": [{"resourceName": "people/123", "names": [{"displayName": "Alice", "givenName": "Alice", "familyName": "A"}], "emailAddresses": [{"value": "alice@example.com"}], "organizations": [{"name": "Example", "title": "CEO"}]}]}
            class FakeMS:
                async def get_contacts(self, top=200, select=None, order_by=None):
                    return {"value": [{"id": "abc", "displayName": "Bob B", "givenName": "Bob", "surname": "B", "emailAddresses": [{"address": "bob@contoso.com", "name": "Bob B"}], "companyName": "Contoso", "jobTitle": "CTO", "businessPhones": ["+1-555-0100"]}]}
    ...
    async def test_list_contacts_cache_hit(self, client, auth_headers):
        with patch("services.office.api.contacts.cache_manager") as mock_cache:
            mock_cache.get_from_cache = AsyncMock(return_value={"contacts": [], "total_count": 0, "providers_used": ["google"]})
