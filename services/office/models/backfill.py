...
class BackfillRequest(BaseModel):
    ...
    start_date: Optional[datetime] = Field(
        None, description="Start date for email range"
    )
    end_date: Optional[datetime] = Field(None, description="End date for email range")
    folders: Optional[List[str]] = Field(
        None, description="Specific folders to backfill"
    )
    batch_size: Optional[int] = Field(
        100, ge=1, le=1000, description="Batch size for processing"
    )
    rate_limit: Optional[int] = Field(
        100, ge=1, le=1000, description="Emails per second limit"
    )
    include_attachments: bool = Field(
        False, description="Whether to include attachment metadata"
    )
    include_deleted: bool = Field(
        False, description="Whether to include deleted emails"
    )
    max_emails: Optional[int] = Field(
        None, ge=1, le=10000, description="Maximum emails to process"
    )
...
class BackfillStatus(BaseModel):
    ...
    resume_time: Optional[datetime] = Field(
        None, description="When the job was resumed"
    )
...
class BackfillJobSummary(BaseModel):
    ...
    total_emails_processed: int = Field(
        ..., description="Total emails processed across all jobs"
    )
    total_emails_failed: int = Field(
