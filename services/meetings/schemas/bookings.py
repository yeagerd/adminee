...
from pydantic import BaseModel, Field, validator, EmailStr, model_validator, ConfigDict
...
class BookingSettings(BaseModel):
    ...
    buffer_before: int = Field(0, ge=0, le=120, description="Buffer before meeting in minutes")
    buffer_after: int = Field(0, ge=0, le=120, description="Buffer after meeting in minutes")
    ...
    advance_days: int = Field(1, ge=0, le=30, description="Minimum days in advance for booking")
    max_advance_days: int = Field(90, ge=1, le=365, description="Maximum days in advance for booking")
    last_minute_cutoff: int = Field(2, ge=0, le=24, description="Hours before meeting when booking closes")
    ...
    holiday_exclusions: List[str] = Field(
        ...
        description="List of holiday dates in YYYY-MM-DD format"
    )

    @validator('max_advance_days')
    def validate_advance_window(cls, v, values):
        if 'advance_days' in values and v < values['advance_days']:
            raise ValueError('max_advance_days must be greater than or equal to advance_days')
        return v

    @validator('holiday_exclusions')
    def validate_holiday_format(cls, v):
        for date_str in v:
            try:
...
class QuestionField(BaseModel):
    ...
    label: str = Field(..., min_length=1, max_length=200, description="Display label for the question")
    required: bool = Field(False, description="Whether this question is required")
    type: str = Field(..., description="Question type: text, email, textarea, select, phone, number")
    options: Optional[List[str]] = Field(None, description="Options for select type questions")
    placeholder: Optional[str] = Field(None, max_length=100, description="Placeholder text for the input")
    validation: Optional[str] = Field(None, description="Validation rule (e.g., email, phone, url)")

    @validator('id')
    def validate_id_format(cls, v):
        if not re.match(r'^[a-zA-Z][a-zA-Z0-9_-]*$', v):
            raise ValueError('ID must start with a letter and contain only alphanumeric characters, hyphens, and underscores')
        return v
...
class CreateBookingLinkRequest(BaseModel):
    title: str = Field(..., min_length=1, max_length=100, description="Title for the booking link")
    description: Optional[str] = Field(None, max_length=500, description="Description of the booking link")
    slug: Optional[str] = Field(None, max_length=64, description="Custom slug for the URL")
    settings: BookingSettings = Field(..., description="Booking configuration settings")
    template_name: Optional[str] = Field(None, max_length=100, description="Name for the template")
    questions: List[QuestionField] = Field(default_factory=list, description="Custom questions for recipients")
    ...
    @model_validator(mode='after')
    def validate_questions_limit(self):
...
class UpdateBookingLinkRequest(BaseModel):
    ...
    settings: Optional[Dict[str, Any]] = Field(None, description="Partial settings update")
...
class CreateOneTimeLinkRequest(BaseModel):
    ...
    recipient_name: str = Field(..., min_length=1, max_length=100, description="Name of the recipient")
    expires_in_days: int = Field(7, ge=1, le=365, description="Days until the link expires")
...
class CreatePublicBookingRequest(BaseModel):
    start: datetime = Field(..., description="Start time of the meeting")
    end: datetime = Field(..., description="End time of the meeting")
    attendee_email: EmailStr = Field(..., description="Email address of the attendee")
    answers: Dict[str, str] = Field(default_factory=dict, description="Answers to template questions")
...
class CreateTemplateRequest(BaseModel):
    ...
    questions: List[QuestionField] = Field(default_factory=list, description="Questions for the template")
    email_followup_enabled: bool = Field(False, description="Whether to enable follow-up emails")
...
class ErrorResponse(BaseModel):
    type: str = Field(..., description="Error type (e.g., validation_error, not_found)")
