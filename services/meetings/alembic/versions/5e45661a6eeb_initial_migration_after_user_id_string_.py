"""Initial migration after user_id string change

Revision ID: 5e45661a6eeb
Revises:
Create Date: 2025-07-24 21:47:20.213969
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "meeting_type",
            sa.Enum("in_person", "virtual", "tbd", name="meetingtype"),
            nullable=True,
        ),
        sa.Column(
            "status",
            sa.Enum("draft", "active", "closed", "scheduled", name="pollstatus"),
            nullable=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "responded", "declined", name="participantstatus"),
            nullable=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "response",
            sa.Enum("available", "unavailable", "maybe", name="responsetype"),
            nullable=False,
        ),
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(
            ["participant_id"], ["poll_participants.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["poll_id"], ["meeting_polls.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
