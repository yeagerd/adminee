...
async def get_user_availability(user_id: str, start: str, end: str, duration: int) -> dict:
...
async def create_calendar_event(user_id: str, poll_id: str, selected_slot_id: str, participants: List[str]) -> dict:
    url = f"{OFFICE_SERVICE_URL}/calendar/create-meeting"
    headers = {"X-API-Key": API_KEY, "X-User-Id": user_id}
    data = {"pollId": poll_id, "selectedSlotId": selected_slot_id, "participants": participants}
    async with httpx.AsyncClient() as client:
