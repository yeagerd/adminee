...
class TestSecurityUtils:
    ...
    def test_validate_token_format(self):
        ...
        assert (
            SecurityUtils.validate_token_format("bl_abc123def45678901") == False
        )  # Too long
        assert (
            SecurityUtils.validate_token_format("ot_abc123def4567890abcde") == False
        )  # Too long
...
class TestRateLimiter:
    ...
    def test_rate_limit_basic(self):
        ...
        for i in range(5):
            assert (
                check_rate_limit(client_key, max_requests=5, window_seconds=3600)
                == True
            )

        # Should block 6th request
        assert (
            check_rate_limit(client_key, max_requests=5, window_seconds=3600) == False
        )
    ...
    def test_rate_limit_window(self):
        ...
        for i in range(3):
            assert (
                check_rate_limit(client_key, max_requests=3, window_seconds=1) == True
            )
