...
from ..models import MeetingPoll as MeetingPollModel, PollParticipant as PollParticipantModel, get_session
...
async def send_invitations(poll_id: UUID, request: Request):
    with get_session() as session:
        ...
        participants = session.query(PollParticipantModel).filter_by(poll_id=poll_id).all()
        ...
        for participant in participants:
            await email_integration.send_invitation_email(participant.email, subject, body, user_id)
