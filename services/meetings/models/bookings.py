...
class OneTimeLink(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    booking_link_id = Column(UUID(as_uuid=True), ForeignKey("booking_links.id", ondelete="CASCADE"), nullable=False)
...
class Booking(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    link_id = Column(UUID(as_uuid=True), ForeignKey("booking_links.id", ondelete="SET NULL"))
    one_time_link_id = Column(UUID(as_uuid=True), ForeignKey("one_time_links.id", ondelete="SET NULL"))
...
class AnalyticsEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    link_id = Column(UUID(as_uuid=True), ForeignKey("booking_links.id", ondelete="CASCADE"), nullable=False)
