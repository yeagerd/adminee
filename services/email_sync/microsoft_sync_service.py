...
def process_microsoft_notification(message: Any) -> None:
    ...
    try:
        ...
        user_email = data.get('user_email', 'user@example.com')  # TODO: Extract from notification
        ...
        logging.info(f"Fetched {len(emails)} emails from Microsoft Graph for {user_email}")
        ...
        for email in emails:
            ...
            if email.get('receivedDateTime'):
                try:
                    ...
                    email_timestamp = datetime.fromisoformat(email['receivedDateTime'].replace('Z', '+00:00'))
        ...
        logging.info(f"Successfully processed {processed_count} new emails for {user_email}")
        message.ack()
