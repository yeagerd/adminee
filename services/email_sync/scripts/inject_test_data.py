...
try:
    ...
    print("Error: google-cloud-pubsub not installed. Install with: pip install google-cloud-pubsub")
    sys.exit(1)
    ...
    def create_topics_and_subscriptions(self):
        ...
        for sub_name, sub_path in self.subscriptions.items():
            try:
                ...
                topic_key = sub_name.replace("_notifications", "_notifications").replace("_events", "_events")
        ...
        for i, payload in enumerate(webhook_payloads, 1):
            try:
                ...
                future = self.publisher.publish(
                    self.topics["gmail_notifications"],
                    data=message_data
                )
    ...
    def publish_microsoft_webhooks(self):
        ...
        for i, payload in enumerate(webhook_payloads, 1):
            try:
                message_data = json.dumps(payload).encode("utf-8")
                future = self.publisher.publish(
                    self.topics["microsoft_notifications"],
                    data=message_data
                )
                message_id = future.result()
        ...
        def callback(message):
            ...
            try:
                ...
                if "package-tracker-events" in topic_name:
                    ...
                    print(f"📦 Package event: {data.get('carrier', 'Unknown')} - {data.get('tracking_number', 'No tracking')}")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Inject test data into pubsub emulator")
    parser.add_argument("--gmail", action="store_true", help="Publish Gmail webhook notifications")
    parser.add_argument("--microsoft", action="store_true", help="Publish Microsoft webhook notifications")
    parser.add_argument("--emails", action="store_true", help="Publish email data")
    parser.add_argument("--listen", type=int, default=0, help="Listen for events for N seconds")
    parser.add_argument("--all", action="store_true", help="Publish all test data")
    parser.add_argument("--setup", action="store_true", help="Create topics and subscriptions")
