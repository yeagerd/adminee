...
def test_fetch_emails_since_history_id(mock_build):
    ...
    # Mock empty response
    mock_service.users.return_value.history.return_value.list.return_value.execute.return_value = {'history': []}
...
class TestGmailAPIClientIntegration(BaseSelectiveHTTPIntegrationTest):
    ...
    def test_real_gmail_api_integration_mocked(self, gmail_client):
        ...
        gmail_client._mock_service.users.return_value.history.return_value.list.return_value.execute.return_value = mock_history_response
        ...
        def mock_get_message(user_id, id, format):
            if id == 'msg1':
                return MagicMock(execute=lambda: mock_message1)
            elif id == 'msg2':
                return MagicMock(execute=lambda: mock_message2)
            else:
                return MagicMock(execute=lambda: {'id': id, 'threadId': 'thread3', 'payload': {'headers': [], 'body': {'data': ''}}})
        
        gmail_client._mock_service.users.return_value.messages.return_value.get.side_effect = mock_get_message
    ...
    def test_gmail_api_rate_limit_handling_mocked(self, gmail_client):
        ...
        gmail_client._mock_service.users.return_value.history.return_value.list.return_value.execute.side_effect = rate_limit_error
    ...
    def test_gmail_api_authentication_error_handling_mocked(self, gmail_client):
        ...
        gmail_client._mock_service.users.return_value.history.return_value.list.return_value.execute.side_effect = auth_error
    ...
    def test_gmail_api_pagination_handling_mocked(self, gmail_client):
        ...
        gmail_client._mock_service.users.return_value.messages.return_value.get.return_value.execute.return_value = mock_message
    ...
    def test_gmail_api_network_error_handling_mocked(self, gmail_client):
        ...
        # Mock network error
        gmail_client._mock_service.users.return_value.history.return_value.list.return_value.execute.side_effect = Exception("Network error")
    ...
    def test_gmail_api_fetch_with_different_user_ids(self, gmail_client):
        ...
        gmail_client._mock_service.users.return_value.history.return_value.list.return_value.execute.return_value = {'history': []}
        
        # Test with different user IDs
        result1 = gmail_client.fetch_emails_since_history_id("me", "12345")
        result2 = gmail_client.fetch_emails_since_history_id("user@example.com", "12345")
        ...
        calls = gmail_client._mock_service.users.return_value.history.return_value.list.call_args_list
    ...
    def test_gmail_api_fetch_with_retry_parameters(self, gmail_client):
        ...
        result1 = gmail_client.fetch_emails_since_history_id("me", "12345", max_retries=3)
        result2 = gmail_client.fetch_emails_since_history_id("me", "12345", max_retries=5)
