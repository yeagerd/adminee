...
class EmailTrackingSettings(BaseSettings):
    ...
    DATABASE_URL: str = Field("sqlite:///email_tracking.db", description="Database URL for tracking")
...
class EmailProcessingState:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'EmailProcessingState':
        ...
        state.last_processed_timestamp = datetime.fromisoformat(data['last_processed_timestamp']) if data.get('last_processed_timestamp') else None
...
class EmailTrackingService:
    ...
    def get_processing_state(self, user_email: str, provider: str) -> Optional[EmailProcessingState]:
        """Get the processing state for a user and provider."""
        states = self._load_states()
        key = self._get_state_key(user_email, provider)
        return states.get(key)
    
    def update_processing_state(self, user_email: str, provider: str, 
        ...
        logging.info(f"Updated processing state for {user_email} ({provider}): {state.to_dict()}")
    ...
    def mark_email_processed(self, user_email: str, provider: str, email_id: str, 
