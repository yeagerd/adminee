...
class MicrosoftGraphClient:
    ...
    def _fetch_email_by_resource(
        self, resource: str, max_retries: int = 5
    ...
    def get_delta_emails(
        self, delta_link: Optional[str] = None, max_retries: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                for item in result.get("value", []):
                    if (
                        "@odata.type" in item
                        and item["@odata.type"] == "#microsoft.graph.message"
                    ):
                        email_data = {
                            "id": item["id"],
                            "subject": item.get("subject", ""),
                            "from": self._extract_email_address(item.get("from", {})),
                            "to": self._extract_email_addresses(
                                item.get("toRecipients", [])
                            ),
