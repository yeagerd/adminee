...
class TestGmailPipelineIntegration:
    ...
    def test_gmail_webhook_to_email_processing_pipeline(self, mock_gmail_client, mock_publish):
        ...
        with app.test_client() as client:
            ...
            for email in [ups_tracking_email(), amazon_shipped_email()]:
                ...
                with patch("services.email_sync.email_parser_service.publish_message") as mock_parser_publish:
                    ...
                    if "1Z999AA1234567890" in email_data["body"]:
...
class TestMicrosoftPipelineIntegration:
    ...
    def test_microsoft_webhook_to_email_processing_pipeline(self, mock_graph_client, mock_publish):
...
class TestEmailParserIntegration:
    ...
    def test_tracking_number_extraction_pipeline(self):
        ...
        with patch("services.email_sync.email_parser_service.publish_message") as mock_publish:
            # Test UPS tracking email
            email_data = {
                "from": "UPS <noreply@ups.com>",
                "body": "Your package has been shipped and is on its way!<br><br>Tracking Number: 1Z999AA1234567890<br>Estimated Delivery: January 17, 2024"
            }
            ...
            assert call_args[0][0] == "package-tracker-events"
            assert call_args[0][1]["carrier"] == "UPS"
            assert call_args[0][1]["tracking_number"] == "1Z999AA1234567890"
    ...
    def test_survey_url_extraction_pipeline(self):
        ...
        with patch("services.email_sync.email_parser_service.publish_message") as mock_publish:
            ...
            call_args = mock_publish.call_args
            assert call_args[0][0] == "survey-events"
            assert call_args[0][1]["survey_url"] == "https://survey.ourapp.com/response/abc123"
...
class TestPipelineErrorHandling:
    ...
    def test_email_parser_pubsub_failure_handling(self):
        ...
        with patch("services.email_sync.email_parser_service.publish_message", side_effect=Exception("pubsub error")) as mock_publish:
...
class TestPipelinePerformance:
    ...
    def test_bulk_email_processing(self):
        ...
        with patch("services.email_sync.email_parser_service.publish_message") as mock_publish:
            ...
            assert mock_publish.call_count >= 0  # At least some emails should trigger events
