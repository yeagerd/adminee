...
from .models import RoutingResult, DownstreamService
...
class OfficeRouter:
    ...
    async def route_email(self, email_data: Dict[str, Any]) -> RoutingResult:
        ...
        logger.info(f"Routing email {email_data.get('id', 'unknown')} to downstream services")
        ...
        if routing_tasks:
            service_results = await asyncio.gather(*routing_tasks, return_exceptions=True)
            
            # Process results
            for i, (service_name, service) in enumerate(self.downstream_services.items()):
                if service.enabled:
    ...
    async def route_calendar(self, calendar_data: Dict[str, Any]) -> RoutingResult:
        ...
        logger.info(f"Routing calendar event {calendar_data.get('id', 'unknown')} to downstream services")
        ...
        if self.downstream_services["vespa"].enabled:
            task = self._route_to_service("vespa", self.downstream_services["vespa"], calendar_data)
            routing_tasks.append(task)
        
        # Route to notifications for calendar reminders
        if self.downstream_services["notifications"].enabled:
            task = self._route_to_service("notifications", self.downstream_services["notifications"], calendar_data)
            routing_tasks.append(task)
        ...
        if routing_tasks:
            ...
            for i, result in enumerate(service_results):
                ...
                if isinstance(result, Exception):
                    logger.error(f"Failed to route calendar to {service_name}: {result}")
                    results[service_name] = {
                        "status": "error",
                        "error": str(result)
                    }
    ...
    async def route_contact(self, contact_data: Dict[str, Any]) -> RoutingResult:
        ...
        logger.info(f"Routing contact {contact_data.get('id', 'unknown')} to downstream services")
        ...
        if self.downstream_services["vespa"].enabled:
            task = self._route_to_service("vespa", self.downstream_services["vespa"], contact_data)
            routing_tasks.append(task)
        
        if self.downstream_services["contacts"].enabled:
            task = self._route_to_service("contacts", self.downstream_services["contacts"], contact_data)
            routing_tasks.append(task)
    ...
    async def _route_to_service(self, service_name: str, service: DownstreamService, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(f"Service {service_name} returned {response.status}: {error_text}")
                    return {
                        "status": "error",
    ...
    def _prepare_data_for_service(self, service_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
