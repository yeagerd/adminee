...
class PubSubConsumer:
    ...
    async def _start_email_subscription(self):
        ...
        subscription = self.subscriber.subscribe(
            ...
            flow_control=pubsub_v1.types.FlowControl(
                max_messages=100, max_bytes=1024 * 1024  # 1MB
            ),
        )
    ...
    async def _start_calendar_subscription(self):
        ...
        subscription = self.subscriber.subscribe(
            ...
            flow_control=pubsub_v1.types.FlowControl(
                max_messages=50, max_bytes=512 * 1024  # 512KB
            ),
        )
    ...
    def _email_message_callback(self, message):
        ...
        try:
            ...
            logger.info(
                f"Processing email message {self.message_count}: {data.get('id', 'unknown')}"
            )
    ...
    def _calendar_message_callback(self, message):
        ...
        try:
            ...
            data = json.loads(message.data.decode("utf-8"))
            logger.info(
                f"Processing calendar message {self.message_count}: {data.get('id', 'unknown')}"
            )
        ...
        try:
            ...
            for attempt in range(self.settings.max_retries):
                try:
                    ...
                    logger.info(
                        f"Successfully routed email {data.get('id', 'unknown')} on attempt {attempt + 1}"
                    )
                    return result
                except Exception as e:
                    if attempt == self.settings.max_retries - 1:
                        logger.error(
                            f"Failed to route email {data.get('id', 'unknown')} after {self.settings.max_retries} attempts: {e}"
                        )
                        raise
                    else:
                        logger.warning(
                            f"Attempt {attempt + 1} failed for email {data.get('id', 'unknown')}: {e}"
                        )
                        await asyncio.sleep(
        ...
        try:
            ...
            for attempt in range(self.settings.max_retries):
                try:
                    ...
                    logger.info(
                        f"Successfully routed calendar event {data.get('id', 'unknown')} on attempt {attempt + 1}"
                    )
                    return result
                except Exception as e:
                    if attempt == self.settings.max_retries - 1:
                        logger.error(
                            f"Failed to route calendar event {data.get('id', 'unknown')} after {self.settings.max_retries} attempts: {e}"
                        )
                        raise
                    else:
                        logger.warning(
                            f"Attempt {attempt + 1} failed for calendar event {data.get('id', 'unknown')}: {e}"
                        )
                        await asyncio.sleep(
                            self.settings.retry_delay_seconds * (2**attempt)
                        )  # Exponential backoff
    ...
    def get_subscription_status(self) -> Dict[str, Any]:
        ...
        return {
            "email": {
                "active": "email" in self.subscriptions
                and self.subscriptions["email"] is not None,
                "subscription_path": f"projects/{self.settings.pubsub_project_id}/subscriptions/{self.settings.pubsub_email_subscription}",
            },
            "calendar": {
                "active": "calendar" in self.subscriptions
                and self.subscriptions["calendar"] is not None,
            ...
            "stats": {
                ...
                "success_rate": (
                    (self.message_count - self.error_count) / max(self.message_count, 1)
                )
                * 100,
            },
        }
