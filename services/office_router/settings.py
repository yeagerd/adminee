...
class Settings(BaseSettings):
    ...
    pubsub_email_topic: str = Field(
        default="email-backfill",
        env="PUBSUB_EMAIL_TOPIC"
    )
    pubsub_email_subscription: str = Field(
        default="email-router-subscription",
        env="PUBSUB_EMAIL_SUBSCRIPTION"
    )
    pubsub_calendar_topic: str = Field(
        default="calendar-updates",
        env="PUBSUB_CALENDAR_TOPIC"
    )
    pubsub_calendar_subscription: str = Field(
        default="calendar-router-subscription",
        env="PUBSUB_CALENDAR_SUBSCRIPTION"
    )
