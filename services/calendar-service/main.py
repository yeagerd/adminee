...
async def list_calendar_events(
    ...
    provider_type: str = Query(..., description="The calendar provider to use (e.g., 'microsoft', 'google')"),
    start_date: Optional[str] = Query(None, description="Start date in YYYY-MM-DD format (user's local time)"),
    end_date: Optional[str] = Query(None, description="End date in YYYY-MM-DD format (user's local time)"),
    user_timezone: Optional[str] = Query("UTC", description="User's IANA timezone ID (e.g., America/New_York). Defaults to UTC.")
):
    """
    ...
    if not ProviderClass:
        raise HTTPException(status_code=400, detail=f"Unsupported provider_type: {provider_type}. Supported types are: {list(PROVIDER_MAP.keys())}")
    
    provider_instance = ProviderClass() # Instantiate the chosen provider

    final_user_timezone = user_timezone if user_timezone and user_timezone.strip() else "UTC"
    ...
    if start_date is None and end_date is None:
        ...
        start_dt_obj = datetime(today_date.year, today_date.month, today_date.day, 0, 0, 0)
        end_dt_obj = datetime(today_date.year, today_date.month, today_date.day, 23, 59, 59, 999999)
    else:
        if start_date:
            try:
                parsed_date = datetime.strptime(start_date, "%Y-%m-%d")
                start_dt_obj = datetime(parsed_date.year, parsed_date.month, parsed_date.day, 0, 0, 0)
            except ValueError:
        ...
        if end_date:
            try:
                ...
                end_dt_obj = datetime(parsed_date.year, parsed_date.month, parsed_date.day, 23, 59, 59, 999999)
            except ValueError:
        ...
        if (start_dt_obj and not end_dt_obj) or (not start_dt_obj and end_dt_obj):
            raise HTTPException(status_code=400, detail="If providing a date range, both start_date and end_date are required.")
    ...
    if start_dt_obj >= end_dt_obj: # This check is now implicitly for non-None dates
        raise HTTPException(status_code=400, detail="If both dates are provided, end_date must be after start_date.")
    ...
    # or make the providers raise more generic exceptions defined in base or exceptions.py.
        ...
        if e.graph_error_details and e.graph_error_details.get('error', {}).get('message'):
             detail_msg += f" Provider details: {e.graph_error_details['error']['message']}"
        raise HTTPException(status_code=e.status_code if e.status_code else 401, detail=detail_msg)
        ...
        if e.graph_error_details and e.graph_error_details.get('error', {}).get('message'):
             detail_msg += f" Provider details: {e.graph_error_details['error']['message']}"
        raise HTTPException(status_code=e.status_code if e.status_code else 400, detail=detail_msg)
    except (GraphAPIServerError, GraphAPIDecodingError, GraphClientError) as e:
        print(f"Unhandled Provider Client or Server Error ({provider_type}): {type(e).__name__} - {str(e)}") 
        raise HTTPException(status_code=502, detail=f"An error occurred while communicating with the {provider_type} calendar service.")
    except Exception as e:
        print(f"Unexpected server error in list_calendar_events ({provider_type}): {type(e).__name__} - {str(e)}")
