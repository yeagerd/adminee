...
class EmailAddress(BaseModel):
    ...
    address: Optional[str] = None # Address can sometimes be null for certain attendees/organizers

class ResponseStatus(BaseModel):
    response: Optional[str] = None # e.g., "accepted", "declined", "tentativelyAccepted", "notResponded", "none"
...
class Attendee(BaseModel):
    emailAddress: Optional[EmailAddress] = None # Changed from dict to EmailAddress model
...
class CalendarEvent(BaseModel):
    ...
    attendees: List[Attendee] = Field(default_factory=list) # Ensure it's List, not Optional[List]
...
class AttendeeStatusEnum(str, Enum):
    ...
    NONE = "none" # For organizer or if status is genuinely missing/unknown from provider
...
class WorkDay(BaseModel):
    ...
    end_time: Optional[str] = Field(None, pattern=r"^([01]\d|2[0-3]):([0-5]\d)$")   # HH:MM format
...
class WorkHoursConflictInfo(BaseModel):
    ...
    conflict_details: Optional[str] = None # e.g., "Starts before work hours", "Ends after work hours", "During non-working day"
