...
# Optional authentication for services that might not always need it
async def optional_service_auth(
    service_name: str,
    request: Request
) -> Optional[str]:
    """Optional authentication from a specific service."""
    api_key = await get_api_key_from_header(request)
    if not api_key:
        return None
...
# Convenience functions for optional authentication
async def optional_user_service_auth(request: Request) -> Optional[str]:
    """Optional authentication from User Service."""
    return await optional_service_auth("user", request)


async def optional_chat_service_auth(request: Request) -> Optional[str]:
    """Optional authentication from Chat Service."""
    return await optional_service_auth("chat", request)


async def optional_meetings_service_auth(request: Request) -> Optional[str]:
    """Optional authentication from Meetings Service."""
    return await optional_service_auth("meetings", request)


async def optional_shipments_service_auth(request: Request) -> Optional[str]:
    """Optional authentication from Shipments Service."""
    return await optional_service_auth("shipments", request)
