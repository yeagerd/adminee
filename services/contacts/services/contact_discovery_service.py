...
class ContactDiscoveryService:
    ...
    async def process_email_event(self, event: EmailEvent, session: AsyncSession) -> None:
    ...
    async def process_calendar_event(self, event: CalendarEvent, session: AsyncSession) -> None:
    ...
    async def process_document_event(self, event: DocumentEvent, session: AsyncSession) -> None:
    ...
    async def _process_todo_shared_contacts(self, event: TodoEvent, session: AsyncSession) -> None:
    ...
    async def get_contact(self, session: AsyncSession, user_id: str, email: str) -> Optional[Contact]:
        """Get a contact by user ID and email."""
        try:
            result = await session.execute(
                select(Contact).where(
                    Contact.user_id == user_id,
                    Contact.email_address == email.lower()
                )
            )
    ...
    async def get_user_contacts(self, session: AsyncSession, user_id: str, limit: int = 100) -> List[Contact]:
    ...
    async def update_contact(
        self, session: AsyncSession, user_id: str, email: str, update_data: EmailContactUpdate
    ) -> Optional[Contact]:
    ...
    async def remove_contact(self, session: AsyncSession, user_id: str, email: str) -> bool:
        """Remove a contact."""
        try:
            contact = await self.get_contact(session, user_id, email)
            if contact:
