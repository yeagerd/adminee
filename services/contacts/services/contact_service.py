...
class ContactService:
    ...
    async def create_contact(
        ...
        try:
            ...
            logger.info(f"Created new contact: {contact.email_address} for user {contact.user_id}")
            return contact
        ...
        try:
            ...
            if source_services:
                ...
                for service in source_services:
                    service_conditions.append(Contact.source_services.contains([service]))
                query = query.where(func.or_(*service_conditions))
            
            # Order by relevance score and apply pagination
            query = query.order_by(Contact.relevance_score.desc()).offset(offset).limit(limit)
    ...
    async def search_contacts(
        ...
        try:
            ...
            base_query = base_query.order_by(Contact.relevance_score.desc()).limit(limit)
    ...
    async def get_contact_stats(
        ...
        try:
            ...
            # Get total events
            result = await session.execute(
                select(func.sum(Contact.total_event_count)).where(Contact.user_id == user_id)
            )
