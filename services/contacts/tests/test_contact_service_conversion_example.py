...
class TestContactServiceConversionExample(PostgresTestDB):
    ...
    @pytest.mark.asyncio
    async def test_create_contact_success_real_db(self, session: AsyncSession, contact_service: ContactService):
        ...
        try:
            ...
            result = await session.execute(
                select(Contact).where(Contact.email_address == contact_data["email_address"])
            )
    ...
    @pytest.mark.asyncio
    async def test_get_contact_by_email_real_db(self, session: AsyncSession, contact_service: ContactService):
    ...
    @pytest.mark.asyncio
    async def test_update_contact_real_db(self, session: AsyncSession, contact_service: ContactService):
