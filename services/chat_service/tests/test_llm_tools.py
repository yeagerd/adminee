...
def test_get_calendar_events_success(monkeypatch):
    ...
    result = get_calendar_events("token123", start_date="2025-06-05", end_date="2025-06-06", user_timezone="UTC", provider_type="google")
...
def test_get_notes_success(monkeypatch):
    ...
    monkeypatch.setattr(requests, "get", mock_get)
    result = get_notes("token123", notebook="work", tags="important", search_query="project", max_results=5)
...
def test_get_documents_success(monkeypatch):
    ...
    monkeypatch.setattr(requests, "get", mock_get)
    result = get_documents("token123", document_type="word", search_query="project", max_results=10)
    assert "documents" in result
    assert result["documents"][0]["title"] == "Test Document"

def test_create_draft_email():
    result = create_draft_email(thread_id="thread123", to="test@example.com", subject="Test", body="Body")
...
def test_create_draft_calendar_event():
    result = create_draft_calendar_event(thread_id="thread123", title="Meeting", start_time="2025-06-07T10:00:00Z", end_time="2025-06-07T11:00:00Z")
    assert result["success"] is True
    assert "draft" in result
...
def test_create_draft_calendar_change():
    result = create_draft_calendar_change(thread_id="thread123", event_id="event456", change_type="reschedule", new_start_time="2025-06-08T10:00:00Z", new_end_time="2025-06-08T11:00:00Z")
    assert result["success"] is True
    assert "draft" in result
    assert result["draft"]["event_id"] == "event456"

def test_delete_draft_calendar_change():
    create_draft_calendar_change(thread_id="thread123", event_id="event456", change_type="cancel")
...
def test_tool_registry(monkeypatch):
    ...
    # Calendar
    calendar_result = registry.execute_tool("get_calendar_events", user_token="user_token")
    assert "events" in calendar_result.raw_output
    # Email
    email_result = registry.execute_tool("get_emails", user_token="user_token", unread_only=True)
    ...
    # Draft email
    draft_result = registry.execute_tool("create_draft_email", thread_id="thread", to="colleague@example.com", subject="Subject", body="Body")
    assert draft_result.raw_output.get("success") is True
    # Draft calendar event
    event_result = registry.execute_tool("create_draft_calendar_event", thread_id="thread", title="Meeting", start_time="2025-06-08T10:00:00Z", end_time="2025-06-08T11:00:00Z")
    assert event_result.raw_output.get("success") is True
    # Delete drafts
    delete_email_result = registry.execute_tool("delete_draft_email", thread_id="thread")
    assert delete_email_result.raw_output.get("success") is True
    delete_event_result = registry.execute_tool("delete_draft_calendar_event", thread_id="thread")
