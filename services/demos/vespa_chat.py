...
class VespaChatDemo:
    ...
    async def run_chat_demo(self) -> Dict[str, Any]:
        ...
        try:
            ...
            demo_results["performance_metrics"] = self._calculate_performance_metrics(demo_results["conversation_scenarios"])
            
            # Assess search quality
            demo_results["search_quality"] = self._assess_search_quality(demo_results["conversation_scenarios"])
            ...
            logger.info(f"Chat demo completed successfully in {demo_duration:.2f} seconds")
    ...
    async def _run_conversation_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            scenario_result["relevance_score"] = self._calculate_scenario_relevance(scenario_result)
            
            logger.info(f"Scenario '{scenario['name']}' completed with relevance score {scenario_result['relevance_score']:.2f}")
    ...
    async def _process_follow_up_question(self, follow_up: str, initial_search: Dict[str, Any], 
        ...
        try:
            ...
            return {
                ...
                "results": enhanced_results,
                "context_utilization": self._assess_context_utilization(initial_search, enhanced_results),
                "relevance_improvement": self._calculate_relevance_improvement(initial_search, enhanced_results)
            }
    ...
    def _enhance_results_for_chat(self, search_results: Dict[str, Any], query: str) -> Dict[str, Any]:
        ...
        for result_type, results in grouped_results.items():
            enhanced["grouped_results"][result_type] = self._format_results_for_chat(results, result_type)
        
        # Generate chat suggestions
        enhanced["chat_suggestions"] = self._generate_chat_suggestions(query, search_results)
    ...
    def _format_results_for_chat(self, results: List[Dict[str, Any]], result_type: str) -> List[Dict[str, Any]]:
    ...
    def _create_chat_display(self, result: Dict[str, Any], result_type: str) -> str:
        ...
        if result_type == "emails":
            ...
            return f"📅 **{result.get('title', 'No title')}**\n{result.get('snippet', '')}"
    ...
    def _extract_chat_metadata(self, result: Dict[str, Any], result_type: str) -> Dict[str, Any]:
    ...
    def _generate_chat_suggestions(self, query: str, search_results: Dict[str, Any]) -> List[str]:
    ...
    def _generate_quick_actions(self, query: str, search_results: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _enhance_follow_up_results(self, semantic_results: Dict[str, Any], 
    ...
    def _integrate_search_context(self, semantic_results: Dict[str, Any], 
                                initial_search: Dict[str, Any]) -> Dict[str, Any]:
        """Integrate context from initial search into follow-up results"""
        context_integration = {
            "original_query_context": initial_search.get("query", ""),
            "result_types_found": list(initial_search.get("results", {}).get("grouped_results", {}).keys()),
        ...
        if semantic_results.get("results"):
            ...
            context_integration["context_utilization_score"] = overlap / len(initial_types) if initial_types else 0.0
        
        return context_integration
    
    def _improve_relevance_with_context(self, semantic_results: Dict[str, Any], 
                                      initial_search: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for result in semantic_results.get("results", []):
            ...
            if result.get("type") in initial_context:
                improved_result["context_boosted_score"] = result.get("semantic_score", 0.0) * 1.2
                improved_result["context_relevance"] = "high"
            else:
                improved_result["context_boosted_score"] = result.get("semantic_score", 0.0)
                improved_result["context_relevance"] = "medium"
            
            improved_results.append(improved_result)
        
        # Sort by improved relevance
        improved_results.sort(key=lambda x: x.get("context_boosted_score", 0.0), reverse=True)
    ...
    def _calculate_scenario_relevance(self, scenario_result: Dict[str, Any]) -> float:
        ...
        base_relevance = min(initial_search.get("total_found", 0) / 10.0, 1.0)  # Normalize to 0-1
        
        # Boost for follow-up quality
        follow_up_quality = 0.0
        if scenario_result.get("follow_up_conversations"):
            successful_follow_ups = sum(1 for f in scenario_result["follow_up_conversations"] if not f.get("error"))
            follow_up_quality = successful_follow_ups / len(scenario_result["follow_up_conversations"])
    ...
    def _calculate_performance_metrics(self, conversation_scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate performance metrics across all scenarios"""
        total_scenarios = len(conversation_scenarios)
        successful_scenarios = sum(1 for s in conversation_scenarios if not s.get("error"))
        
        # Calculate average search time
        search_times = [s.get("search_time_ms", 0) for s in conversation_scenarios if s.get("search_time_ms")]
        avg_search_time = sum(search_times) / len(search_times) if search_times else 0
        
        # Calculate average relevance score
        relevance_scores = [s.get("relevance_score", 0.0) for s in conversation_scenarios if s.get("relevance_score")]
    ...
    def _assess_search_quality(self, conversation_scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        total_expected_results = sum(len(s.get("expected_results", [])) for s in conversation_scenarios)
        total_actual_results = sum(s.get("total_results", 0) for s in conversation_scenarios)
        quality_metrics["result_coverage"] = min(total_actual_results / max(total_expected_results, 1), 1.0)
        ...
        for scenario in conversation_scenarios:
            for follow_up in scenario.get("follow_up_conversations", []):
                if follow_up.get("context_utilization"):
                    context_scores.append(follow_up["context_utilization"].get("context_utilization_score", 0.0))
        
        if context_scores:
            quality_metrics["context_utilization"] = sum(context_scores) / len(context_scores)
        
        # Calculate follow-up effectiveness
        total_follow_ups = sum(len(s.get("follow_up_conversations", [])) for s in conversation_scenarios)
        successful_follow_ups = sum(
            len([f for f in s.get("follow_up_conversations", []) if not f.get("error")])
            for s in conversation_scenarios
        )
        quality_metrics["follow_up_effectiveness"] = successful_follow_ups / total_follow_ups if total_follow_ups > 0 else 0.0
        
        return quality_metrics
    
    def _evaluate_user_experience(self, conversation_scenarios: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        flow_scores = []
        for scenario in conversation_scenarios:
            if scenario.get("initial_search") and scenario.get("follow_up_conversations"):
                # Check if follow-ups build on initial results
                initial_results = scenario["initial_search"].get("total_found", 0)
                follow_up_quality = len([f for f in scenario["follow_up_conversations"] if not f.get("error")])
        ...
        presentation_scores = []
        for scenario in conversation_scenarios:
            if scenario.get("initial_search", {}).get("results", {}).get("chat_suggestions"):
                # Check if results include helpful suggestions and actions
                suggestions_count = len(scenario["initial_search"]["results"]["chat_suggestions"])
                actions_count = len(scenario["initial_search"]["results"]["quick_actions"])
                presentation_score = min((suggestions_count + actions_count) / 10.0, 1.0)
                presentation_scores.append(presentation_score)
        
        if presentation_scores:
            ux_metrics["result_presentation"] = sum(presentation_scores) / len(presentation_scores)
