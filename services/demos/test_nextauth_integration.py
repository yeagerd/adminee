...
async def test_user_service():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.get(
            "http://localhost:8001/users/google_108234567890123456789",
            headers=headers
        )
        if response.status_code == 200:
            user_data = response.json()
            print(f"✅ User Profile: {user_data['email']} (Provider: {user_data['auth_provider']})")
        else:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ User Preferences: {prefs_data['user_id']} (Timezone: {prefs_data['ui']['timezone']})")
        else:
...
async def test_chat_service():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Chat Service: Message sent successfully")
