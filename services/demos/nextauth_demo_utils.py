        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"OAuth start failed: {response.status_code} {response.text}")
                    return None
...
async def test_nextauth_flow(provider: str = "google") -> Optional[str]:
    ...
    while True:
        token = input("🔑 Paste your NextAuth JWT token (or 'skip' to cancel): ").strip()
        ...
        if verification and verification.get("valid"):
            ...
            if session:
                ...
                print(f"   Access Token: {'✅ Available' if session.get('access_token') else '❌ Not available'}")
                print(f"   Refresh Token: {'✅ Available' if session.get('refresh_token') else '❌ Not available'}")
            
            return token
        else:
            error = verification.get("error", "Unknown error") if verification else "Verification failed"
            print(f"❌ Token verification failed: {error}")
            print("   Please try again or type 'skip' to cancel")


def create_nextauth_jwt_for_demo(user_id: str, email: str, provider: str = "google") -> str:
    """
...
def compare_auth_approaches(clerk_token: Optional[str], nextauth_token: Optional[str]):
    ...
    # Decode both tokens
    clerk_data = decode_token_safely(clerk_token, "Clerk") if clerk_token else {"error": "No Clerk token"}
    nextauth_data = decode_token_safely(nextauth_token, "NextAuth") if nextauth_token else {"error": "No NextAuth token"}
    ...
    for field_name, field_key in fields_to_compare:
        ...
        if field_key in ["iat", "exp"] and isinstance(clerk_value, (int, float)):
            clerk_value = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(clerk_value))
        if field_key in ["iat", "exp"] and isinstance(nextauth_value, (int, float)):
            nextauth_value = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(nextauth_value))
        
        # Truncate long values
        clerk_str = str(clerk_value)[:28] + "..." if len(str(clerk_value)) > 28 else str(clerk_value)
        nextauth_str = str(nextauth_value)[:28] + "..." if len(str(nextauth_value)) > 28 else str(nextauth_value)
...
async def demonstrate_nextauth_integration():
    ...
    # Test the flow
    provider = input("Test OAuth flow? Enter 'google', 'microsoft', or 'skip': ").strip().lower()
