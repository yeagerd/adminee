...
class TestBrieflyAgentContext:
    ...
    def mock_service_client(self):
        ...
        mock_client.get_user_info.return_value = {"id": "test-user-123", "name": "Test User"}
    ...
    @pytest.mark.asyncio
    async def test_conversation_context_loading_and_saving(self, mock_history_manager, mock_service_client, fake_llm):
        """Test that conversation context is loaded and saved correctly."""
        with patch('services.chat.history_manager.get_thread_history', mock_history_manager.get_thread_history), \
             patch('services.chat.agents.llm_manager.get_llm_manager') as mock_llm_manager:
            ...
            with patch.object(type(agent), 'run', return_value=mock_handler):
                ...
                mock_history_manager.get_thread_history.assert_called_once_with(123, limit=100)

    @pytest.mark.asyncio
    async def test_context_persistence_between_calls(self, mock_history_manager, mock_service_client, fake_llm):
        ...
        with patch('services.chat.history_manager.get_thread_history', mock_history_manager.get_thread_history), \
            ...
            with patch.object(type(agent), 'run', return_value=mock_handler) as mock_run:
    ...
    @pytest.mark.asyncio
    async def test_empty_conversation_history(self, mock_history_manager, mock_service_client, fake_llm):
    ...
    @pytest.mark.asyncio
    async def test_database_error_handling(self, mock_history_manager, mock_service_client, fake_llm):
        """Test graceful handling of database errors."""
        with patch('services.chat.history_manager.get_thread_history', mock_history_manager.get_thread_history), \
             patch('services.chat.agents.llm_manager.get_llm_manager') as mock_llm_manager:
            
            # Mock database error
            mock_history_manager.get_thread_history.side_effect = Exception("Database connection failed")
    ...
    @pytest.mark.asyncio
    async def test_conversation_history_loading(self, mock_history_manager, mock_service_client, fake_llm):
