...
def create_draft_email(
    ...
    **kwargs: Any,
    ...
    """Create a draft email for the given thread."""
    # This is a legacy function that doesn't have user_id context
    # It should be replaced with DraftTools.create_draft_email()
    logger.warning("create_draft_email called without user_id context - use DraftTools instead")
    try:
        key = f"{thread_id}_email"
        email_data = {
            "to": to or "",
...
def create_draft_calendar_event(
    ...
    **kwargs: Any,
    ...
    """Create or update a draft calendar event for the given thread."""
    # This is a legacy function that doesn't have user_id context
    # It should be replaced with DraftTools.create_draft_calendar_event()
    logger.warning("create_draft_calendar_event called without user_id context - use DraftTools instead")
    try:
        key = f"{thread_id}_calendar_event"
...
def create_draft_calendar_change(
    ...
    **kwargs: Any,
    ...
    """Create a draft calendar change for the given thread."""
    # This is a legacy function that doesn't have user_id context
    # It should be replaced with DraftTools.create_draft_calendar_change()
    logger.warning("create_draft_calendar_change called without user_id context - use DraftTools instead")
    try:
        # Validate required parameters
        if not event_id or not event_id.strip():
            return {"success": False, "message": "event_id is required"}
