...
class BrieflyAgent(FunctionAgent):
    ...
    async def _load_conversation_history(self, exclude_latest: bool = True) -> None:
        ...
        try:
            ...
            if exclude_latest and db_messages and db_messages[-1].user_id == self._user_id:
                ...
                if hasattr(handler, 'stream_events'):
                    try:
                        ...
                        async for event in handler.stream_events():
                            ...
                            if isinstance(event, AgentStream) and hasattr(event, 'delta') and event.delta:
                    ...
                    if response and not response.startswith("input=["):  # Filter out debug content
                        yield response
                    else:
