...
from services.common.models.document_chunking import (
    DocumentChunk, ChunkingRule, ChunkingResult, ChunkingStrategy, 
    ChunkType, DocumentChunkingConfig
)
...
class DocumentChunkingService:
    ...
    def chunk_document(
        ...
        try:
            ...
            result = ChunkingResult(
                ...
                average_chunk_size=sum(chunk.content_length for chunk in chunks) / len(chunks) if chunks else 0,
                chunk_size_variance=self._calculate_variance([chunk.content_length for chunk in chunks]),
            ...
            if not result.validate_chunk_sequence():
                logger.warning(f"Chunk sequence validation failed for document {document_id}")
    ...
    def _section_boundary_chunking(
        ...
        for i, (section_title, section_content) in enumerate(sections):
            ...
            chunk = DocumentChunk(
                parent_doc_id=metadata.get("document_id", "unknown") if metadata else "unknown",
                chunk_sequence=i + 1,
    ...
    def _fixed_size_chunking(
        ...
        while start < len(content):
            ...
            if end < len(content):
                ...
                if last_space > start + chunk_size * 0.8:  # Only break at space if we're close to target
                    end = last_space
    ...
    def _extract_sections(self, content: str, metadata: Optional[Dict[str, Any]]) -> List[Tuple[str, str]]:
    ...
    def _extract_pages(self, content: str, metadata: Optional[Dict[str, Any]]) -> List[Tuple[str, Dict[str, Any]]]:
        """Extract pages from content."""
        pages = []
        
        # Try to find page breaks
        page_breaks = re.split(r'\f|\n\s*Page\s+\d+\s*\n|\n\s*---\s*Page\s+\d+\s*---\s*\n', content)
    ...
    def _extract_semantic_units(self, content: str, metadata: Optional[Dict[str, Any]]) -> List[Tuple[str, str]]:
    ...
    def _calculate_quality_score(self, chunks: List[DocumentChunk], rules: ChunkingRule) -> float:
