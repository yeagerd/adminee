...
class DocumentChunk(BaseModel):
    ...
    section_path: List[str] = Field(default_factory=list, description="Hierarchical path to this chunk")
    page_number: Optional[int] = Field(None, description="Page number if applicable")
    
    # Chunking info
    chunking_strategy: ChunkingStrategy = Field(..., description="Strategy used to create this chunk")
    ...
    # Boundaries
    start_offset: int = Field(..., description="Character offset from start of parent document")
    end_offset: int = Field(..., description="Character offset to end of chunk in parent document")
    ...
    child_chunk_ids: List[str] = Field(default_factory=list, description="IDs of sub-chunks if this is a container")
    ...
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When this chunk was created")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="When this chunk was last updated")
    ...
    def to_vespa_document(self, user_id: str, provider: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "operation": "create",
            "batch_id": None,
            "tags": [f"chunk_type:{self.chunk_type}", f"strategy:{self.chunking_strategy}"],
            "metadata": {
...
class ChunkingRule(BaseModel):
    ...
    overlap_size: int = Field(default=0, description="Overlap between chunks in characters")
    
    # Content rules
    preserve_sections: bool = Field(default=True, description="Try to keep sections intact")
    preserve_paragraphs: bool = Field(default=True, description="Try to keep paragraphs intact")
    preserve_sentences: bool = Field(default=False, description="Try to keep sentences intact")
    
    # Special handling
    handle_tables: bool = Field(default=True, description="Special handling for table content")
    handle_lists: bool = Field(default=True, description="Special handling for list content")
    handle_images: bool = Field(default=True, description="Special handling for image content")
    
    # Quality thresholds
    min_content_quality: float = Field(default=0.7, description="Minimum content quality score (0.0-1.0)")
    max_empty_chunks: int = Field(default=0, description="Maximum number of empty chunks allowed")
    
    # Performance settings
    max_processing_time: int = Field(default=30, description="Maximum processing time in seconds")
    batch_size: int = Field(default=100, description="Number of chunks to process in a batch")
...
class ChunkingResult(BaseModel):
    ...
    # Quality metrics
    content_coverage: float = Field(..., description="Percentage of original content covered")
    chunk_quality_score: float = Field(..., description="Overall quality score for chunks")
    ...
    chunking_rules: ChunkingRule = Field(..., description="Rules applied during chunking")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When chunking was completed")
...
class DocumentChunkingConfig(BaseModel):
    ...
    # Global settings
    enable_parallel_processing: bool = Field(default=True, description="Enable parallel chunking")
    max_workers: int = Field(default=4, description="Maximum number of worker processes")
    chunk_cache_size: int = Field(default=1000, description="Number of chunks to cache in memory")
    
    # Quality thresholds
    min_chunk_quality: float = Field(default=0.6, description="Minimum quality for chunks to be indexed")
    max_chunk_retries: int = Field(default=3, description="Maximum retries for failed chunking")
    
    # Storage settings
    enable_chunk_compression: bool = Field(default=True, description="Compress chunk content for storage")
    chunk_retention_days: int = Field(default=365, description="Days to retain chunk data")
