"""
Email contact data models for contact discovery and management.
"""
...
class EmailContactEventCount(BaseModel):
    """Count of events for a specific contact and event type."""
    
    event_type: str = Field(..., description="Type of event (email, calendar, document)")
    count: int = Field(default=0, description="Number of events of this type")
    last_seen: datetime = Field(..., description="When this contact was last seen in this event type")
    first_seen: datetime = Field(..., description="When this contact was first seen in this event type")


class EmailContact(BaseModel):
    """Email contact data model with event type counters and last_seen tracking."""
    ...
    total_event_count: int = Field(default=0, description="Total number of events across all types")
    last_seen: datetime = Field(..., description="When this contact was last seen in any event")
    first_seen: datetime = Field(..., description="When this contact was first seen")
    
    # Contact relevance scoring
    relevance_score: float = Field(default=0.0, description="Contact relevance score (0.0 to 1.0)")
    ...
    # Timestamps
    created_at: datetime = Field(..., description="When this contact record was created")
    updated_at: datetime = Field(..., description="When this contact record was last updated")
    ...
    def to_vespa_document(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "event_counts": {k: v.model_dump() for k, v in self.event_counts.items()},
