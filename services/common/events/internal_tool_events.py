...
class LLMChatMessageData(BaseModel):
    ...
    message_type: str = Field(..., description="Message type (text, tool_call, tool_result)")
    content: str = Field(..., description="Message content")
    tokens_used: Optional[int] = Field(None, description="Number of tokens used")
    response_time_ms: Optional[int] = Field(None, description="Response time in milliseconds")
    cost_usd: Optional[float] = Field(None, description="Cost in USD")
    tools_used: List[str] = Field(default_factory=list, description="Tools used in this message")
    tool_results: List[Dict[str, Any]] = Field(default_factory=list, description="Tool execution results")
    conversation_context: List[Dict[str, Any]] = Field(default_factory=list, description="Conversation context")
    user_feedback: Optional[str] = Field(None, description="User feedback on the response")
...
class ShipmentEventData(BaseModel):
    ...
    event_type: str = Field(..., description="Event type (created, shipped, delivered, etc.)")
    ...
    package_details: Dict[str, Any] = Field(default_factory=dict, description="Package details")
...
class MeetingPollData(BaseModel):
    ...
    poll_type: str = Field(..., description="Poll type (single_choice, multiple_choice, etc.)")
    question: str = Field(..., description="Poll question")
    options: List[str] = Field(..., description="Poll options")
    responses: List[Dict[str, Any]] = Field(default_factory=list, description="Poll responses")
    ...
    is_anonymous: bool = Field(default=False, description="Whether responses are anonymous")
    allow_multiple_votes: bool = Field(default=False, description="Whether multiple votes are allowed")
    ...
    results_visible: bool = Field(default=True, description="Whether results are visible")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata")
...
class BookingData(BaseModel):
    ...
    recurring_end_date: Optional[datetime] = Field(None, description="Recurring end date")
