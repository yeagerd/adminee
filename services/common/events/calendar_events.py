...
class CalendarEventData(BaseModel):
    ...
    attachments: List[Dict[str, Any]] = Field(default_factory=list, description="Event attachments")
    color_id: Optional[str] = Field(None, description="Event color identifier")
    html_link: Optional[str] = Field(None, description="Link to view event in provider's UI")
...
class CalendarBatchEvent(BaseEvent):
    ...
    events: List[CalendarEventData] = Field(..., description="List of calendar events in the batch")
