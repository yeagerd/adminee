...
class ContactData(BaseModel):
    ...
    phone_numbers: List[Dict[str, str]] = Field(default_factory=list, description="Phone numbers with types")
    addresses: List[Dict[str, Any]] = Field(default_factory=list, description="Physical addresses")
    organizations: List[Dict[str, str]] = Field(default_factory=list, description="Organizational information")
    birthdays: List[datetime] = Field(default_factory=list, description="Birthday dates")
    ...
    # Additional metadata
    photos: List[Dict[str, Any]] = Field(default_factory=list, description="Contact photos")
...
class ContactBatchEvent(BaseEvent):
    ...
    contacts: List[ContactData] = Field(..., description="List of contacts in the batch")
