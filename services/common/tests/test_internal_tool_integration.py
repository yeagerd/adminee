...
class TestInternalToolEventIntegration:
    ...
    def _create_test_metadata(
        self, source_service: str = "test-service"
    ) -> EventMetadata:
    ...
    def test_internal_tool_event_subscription_configuration(self):
        ...
        for topic in expected_topics:
            assert (
                topic in vespa_topics
            ), f"Topic {topic} not found in vespa_loader configuration"
    ...
    def test_internal_tool_event_batch_processing(self):
        ...
        llm_events = [self._create_test_llm_chat_event() for _ in range(3)]
        shipment_events = [self._create_test_shipment_event() for _ in range(2)]
