...
class TestEventDrivenArchitectureIntegration:
    ...
    def test_end_to_end_flow_new_architecture(self):
        ...
        assert consistency_check[
            "passed"
        ], f"Consistency check failed: {consistency_check.get('error')}"
    ...
    def test_consumer_scaling_and_isolation(self):
        ...
        for service in consumer_services:
            service_events = self._simulate_selective_consumption(
                service, multi_user_events
            )
            ...
            if service == "vespa_loader":
                ...
                assert (
                    len(event_types) >= 3
                ), f"Vespa loader missing event types: {event_types}"
                ...
                assert event_types.issubset(
                    contact_event_types
                ), f"Contact discovery got unexpected events: {event_types}"
            elif service == "meetings_service":
                # Meetings service should only get calendar events
                event_types = set(event.get("event_type") for event in service_events)
                assert event_types == {
                    "calendar"
                }, f"Meetings service got unexpected events: {event_types}"
            elif service == "shipments_service":
                # Shipments service should only get email events
                event_types = set(event.get("event_type") for event in service_events)
                assert event_types == {
                    "email"
                }, f"Shipments service got unexpected events: {event_types}"
    ...
    def test_error_handling_and_retry_mechanisms(self):
        ...
        for scenario in failure_scenarios:
            ...
            if scenario["should_retry"]:
                ...
                ), f"Retryable error not retried: {scenario}"
                assert (
                    result["final_status"] == "success"
                ), f"Retryable error not resolved: {scenario}"
            else:
                assert (
                    result["final_status"] == "failed"
                ), f"Non-retryable error should fail: {scenario}"
                assert (
                    result["error_type"] == scenario["failure_type"]
                ), f"Wrong error type: {scenario}"
    ...
    def test_unified_search_across_document_types(self):
        ...
        for scenario in search_scenarios:
            search_results = self._simulate_unified_search(
                scenario["query"], vespa_docs
            )

            assert search_results[
                "success"
            ], f"Search failed for query: {scenario['query']}"
            assert (
                search_results["total_found"] > 0
            ), f"No results for query: {scenario['query']}"

            # Verify result types
            result_types = set(
                doc.get("document_type") for doc in search_results["results"]
            )
    ...
    def test_contact_discovery_flow_multiple_sources(self):
        ...
        assert (
            len(contact_sources) >= 2
        ), f"Contacts only found from {len(contact_sources)} sources: {contact_sources}"

        # Verify contact relevance scoring
        for contact in discovered_contacts:
            assert (
                contact["relevance_score"] >= 0.0
            ), f"Invalid relevance score: {contact['relevance_score']}"
            assert (
                contact["relevance_score"] <= 1.0
            ), f"Invalid relevance score: {contact['relevance_score']}"
            assert (
                contact["total_event_count"] > 0
            ), f"Contact has no events: {contact['email_address']}"
    ...
    def test_document_chunking_and_fragment_search(self):
        ...
        for doc_type, document in test_documents.items():
            ...
            assert (
                len(chunks) > 1
            ), f"Document {doc_type} should be chunked into multiple pieces"

            # Verify chunk metadata
            for i, chunk in enumerate(chunks):
                assert (
                    chunk["parent_doc_id"] == document["id"]
                ), f"Chunk {i} missing parent reference"
                assert (
                    chunk["chunk_sequence"] == i
                ), f"Chunk {i} has wrong sequence number"
        ...
        for query in search_queries:
            ...
            assert fragment_results[
                "success"
            ], f"Fragment search failed for query: {query}"
            assert (
                len(fragment_results["results"]) > 0
            ), f"No fragments found for query: {query}"

            # Verify fragment relationships
            for result in fragment_results["results"]:
                assert (
                    "parent_doc_id" in result
                ), "Fragment missing parent document reference"
    ...
    def test_parent_child_navigation_search_results(self):
        ...
        parent_docs = [
            ...
            in ["word_document", "sheet_document", "presentation_document"]
        ]
        child_docs = [
            doc
            for doc in search_results["results"]
            if doc.get("document_type") == "document_fragment"
        ]
        ...
        for child_doc in child_docs:
            ...
            parent_doc = next(
                (doc for doc in parent_docs if doc["id"] == parent_id), None
            )
            assert (
                parent_doc
            ), f"Parent document {parent_id} not found for child {child_doc['id']}"
    ...
    def test_data_freshness_tracking(self):
        ...
        for event in freshness_events:
            ...
            assert (
                result["freshness_score"] >= 0.0
            ), f"Invalid freshness score: {result['freshness_score']}"
            assert (
        ...
        fresh_events = [
            event
            for event, result in zip(freshness_events, freshness_results)
        ...
        if stale_events:  # Only if we have stale events
            for event in stale_events:
                assert event.get(
                    "needs_refresh", False
                ), f"Stale event {event['id']} not marked for refresh"
    ...
    def _simulate_event_processing(
    ...
    def _verify_data_consistency(
        self, events: List[Dict[str, Any]], vespa_docs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _simulate_selective_consumption(
        self, service: str, events: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        if service == "vespa_loader":
            ...
            return [
                event
                for event in events
                if event["event_type"] in ["email", "calendar", "contact", "document"]
            ]
        elif service == "meetings_service":
    ...
    def _simulate_error_scenario(
        self, events: List[Dict[str, Any]], scenario: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _simulate_unified_search(
        self, query: str, vespa_docs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Simulate unified search functionality."""
        # Simple search simulation
        results = [
            doc for doc in vespa_docs if query.lower() in doc.get("content", "").lower()
        ]
    ...
    def _simulate_contact_discovery(
        self, events: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for event in events:
            ...
            for email in emails:
                ...
                contacts[email]["event_counts"][event["type"]] = (
                    contacts[email]["event_counts"].get(event["type"], 0) + 1
                )
                contacts[email]["total_event_count"] += 1

                # Calculate relevance score
                contacts[email]["relevance_score"] = min(
                    1.0, contacts[email]["total_event_count"] / 10.0
                )
    ...
    def _simulate_document_chunking(
    ...
    def _simulate_fragment_search(
        self, query: str, chunking_results: Dict[str, List[Dict[str, Any]]]
    ) -> Dict[str, Any]:
        ...
        return [
            {
                ...
                "children": [
                    ...
                    },
                    {
                        "id": "fragment_2",
                        "type": "document_fragment",
                        "content": "Project objectives and goals",
                    },
                    {
                        "id": "fragment_3",
                        "type": "document_fragment",
    ...
    def _generate_hierarchical_vespa_documents(
        self, hierarchical_data: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _simulate_hierarchical_search(
        self, query: str, vespa_docs: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
                    ...
                    if isinstance(event[field], str):
                        if event[field].isdigit():
                            ...
                            dt = datetime.fromtimestamp(
                                int(event[field]), tz=timezone.utc
                            )
                        else:
                            # ISO format or other string format
    ...
    def _calculate_data_freshness(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "last_updated" in event:
            try:
                if isinstance(event["last_updated"], str):
                    if event["last_updated"].isdigit():
                        last_updated = datetime.fromtimestamp(
                            int(event["last_updated"]), tz=timezone.utc
                        )
                    else:
                        last_updated = datetime.fromisoformat(
                            event["last_updated"].replace("Z", "+00:00")
                        )
                else:
                ...
                return {"freshness_score": freshness_score, "age_hours": age_hours}

            except (ValueError, OSError):
                return {"freshness_score": 0.0, "age_hours": float("inf")}
