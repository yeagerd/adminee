...
class TestUUID4PydanticSchema:
    ...
    def test_pydantic_model_creation_with_invalid_uuid(self):
        """Test that a Pydantic model rejects invalid UUIDs based on constraints."""
        invalid_uuid = "not-a-uuid"

        with pytest.raises(ValidationError) as exc_info:
            TestModel(id=invalid_uuid)

        # Pydantic validates against the schema (length and pattern), not custom logic
    ...
    def test_uuid4_direct_instantiation_wrong_version(self):
        """Test that UUID4 class rejects wrong version UUIDs when created directly."""
        import uuid
