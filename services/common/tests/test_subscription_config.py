...
class TestSubscriptionConfig:
    ...
    def test_get_subscription_name(self):
        ...
        assert SubscriptionConfig.get_subscription_name("vespa_loader", "emails") == "vespa-loader-emails"
        assert SubscriptionConfig.get_subscription_name("vespa_loader", "calendars") == "vespa-loader-calendars"
        assert SubscriptionConfig.get_subscription_name("vespa_loader", "contacts") == "vespa-loader-contacts"
        
        # Test contact_discovery service
        assert SubscriptionConfig.get_subscription_name("contact_discovery", "emails") == "contact-discovery-emails"
        assert SubscriptionConfig.get_subscription_name("contact_discovery", "calendars") == "contact-discovery-calendars"
        
        # Test meetings service
        assert SubscriptionConfig.get_subscription_name("meetings", "calendars") == "meetings-calendars"
        
        # Test shipments service
        assert SubscriptionConfig.get_subscription_name("shipments", "emails") == "shipments-emails"
        
        # Test frontend_sse service
        assert SubscriptionConfig.get_subscription_name("frontend_sse", "emails") == "frontend-sse-emails"
        assert SubscriptionConfig.get_subscription_name("frontend_sse", "todos") == "frontend-sse-todos"
        
        # Test fallback for unknown service
        assert SubscriptionConfig.get_subscription_name("unknown_service", "emails") == "unknown_service-emails"
    ...
    def test_get_subscription_config(self):
        ...
        config = SubscriptionConfig.get_subscription_config("contact_discovery", "emails")
        ...
        config = SubscriptionConfig.get_subscription_config("unknown_service", "unknown_topic")
        assert config["ack_deadline_seconds"] == 60
        assert config["retain_acked_messages"] is False
    ...
    def test_get_service_topics(self):
        ...
        assert len(vespa_topics) == 12  # All document types + emails, calendars, contacts, todos
    ...
    def test_validate_subscription_config(self):
        ...
        assert SubscriptionConfig.validate_subscription_config("vespa_loader", "emails") is True
        assert SubscriptionConfig.validate_subscription_config("contact_discovery", "calendars") is True
        assert SubscriptionConfig.validate_subscription_config("meetings", "calendars") is True
        assert SubscriptionConfig.validate_subscription_config("shipments", "emails") is True
        assert SubscriptionConfig.validate_subscription_config("frontend_sse", "todos") is True
        
        # Test invalid configurations
        assert SubscriptionConfig.validate_subscription_config("unknown_service", "emails") is False
        assert SubscriptionConfig.validate_subscription_config("vespa_loader", "unknown_topic") is False
        assert SubscriptionConfig.validate_subscription_config("unknown_service", "unknown_topic") is False
