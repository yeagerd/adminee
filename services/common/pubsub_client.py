...
class PubSubClient:
    ...
    def publish_email_data(self, email_data: Dict[str, Any], topic_name: str = "email-backfill") -> str:
        """Publish email data to the specified topic"""
        return self.publish_message(topic_name, email_data)
    
    def publish_calendar_data(self, calendar_data: Dict[str, Any], topic_name: str = "calendar-updates") -> str:
        """Publish calendar data to the specified topic"""
        return self.publish_message(topic_name, calendar_data)
    
    def publish_contact_data(self, contact_data: Dict[str, Any], topic_name: str = "contact-updates") -> str:
...
class PubSubConsumer:
    ...
    def subscribe(self, topic_name: str, subscription_name: str, callback: Callable, **kwargs):
        ...
        try:
            ...
            logger.info(f"Subscribed to {topic_name} with subscription {subscription_name}")
            return subscription
...
def create_test_message(data_type: str, **kwargs) -> Dict[str, Any]:
    ...
    if data_type == "email":
        ...
        base_message.update({
            ...
            "email_addresses": kwargs.get("email_addresses", ["contact@example.com"])
        })
