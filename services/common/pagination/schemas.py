...
class CursorPaginationRequest(BaseModel):
    ...
    direction: Optional[str] = Field("next", pattern="^(next|prev)$", description="Pagination direction")
...
class CursorPaginationResponse(BaseModel, Generic[T]):
    ...
    prev_cursor: Optional[str] = Field(None, description="Cursor token for previous page")
...
class PaginationMetadata(BaseModel):
    ...
    total_items: Optional[int] = Field(None, description="Total number of items (if available)")
    current_page_size: int = Field(description="Number of items in current page")
    cursor_age: Optional[int] = Field(None, description="Age of cursor token in seconds")
...
class PaginationInfo(BaseModel):
    ...
    supports_cursor_pagination: bool = Field(True, description="Whether cursor pagination is supported")
    supports_offset_pagination: bool = Field(False, description="Whether offset pagination is supported")
    ...
    cursor_expiry_seconds: int = Field(3600, description="Cursor token expiry time in seconds")
