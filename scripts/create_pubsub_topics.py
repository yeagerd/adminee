#!/usr/bin/env python3
"""
Create required Pub/Sub topics for the Vespa backfill demo.
"""
...
def create_topic(project_id: str, topic_name: str, emulator_host: str = "localhost:8085"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Failed to create topic {topic_name}: {response.status_code} - {response.text}")
            return False
...
def main():
    """Create all required Pub/Sub topics"""
    project_id = "briefly-dev"
    emulator_host = "localhost:8085"
    
    print("Creating Pub/Sub topics for Vespa backfill demo...")
    ...
    success_count = 0
    for topic in topics:
        if create_topic(project_id, topic, emulator_host):
            success_count += 1
    
    print(f"\n✅ Successfully created {success_count}/{len(topics)} topics!")
