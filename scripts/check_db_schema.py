...
async def check_and_fix_schema() -> None:
    ...
    async with async_session() as session:
        ...
        if 'normalized_email' not in user_columns:
            ...
            try:
                await session.execute(text('ALTER TABLE users ADD COLUMN normalized_email VARCHAR(255)'))
        ...
        if 'timezone' not in pref_columns:
            ...
            try:
                await session.execute(text('ALTER TABLE user_preferences ADD COLUMN timezone VARCHAR(50) DEFAULT "UTC"'))
                await session.commit()
